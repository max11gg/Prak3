...
cursor = cnxn.cursor()
ingridientId = []
def CloseOrder(adminId, userId, currentIdCheque, endIdBurger, count):
    if count == 0:
    ...
    for row in cursor.execute(f"select * from [Cheque] inner join [User] on [User_ID] = [ID_User] where [ID_Cheque] = {currentIdCheque}"):
        ...
        count = row.Count_Burger
        cost = row.Cost_Burger
        sum = row.Sum_Order
    ...
    for i in range(len(endIdBurger)):
        for row in cursor.execute(f"select * from [Burgers_Ingridient] where [Burgers_ID] = {endIdBurger[i]}"):
    ...
    file.write(f"Заказ №{currentIdCheque}\n"
               ...
               "Состав заказа: \n"
               "\n"
               f"Бургер: {count} шт., {cost} руб. за шт.\n"
               "Ингридиенты: \n")
